...
/* 
    ...
    state 안에는 변수의 유효범위와 무관하게 계속 유지되어야 하는 전역변수값 설정

    리액트 이벤트 부여방식. JSX 함수로 태그에 인라인방식으로 on+이벤트명 부여가능
*/
/* 부모컴포넌트로부터 전달받은 props를 전역변수에 저장 후 버튼 클릭 시 
    state에 담긴 값을 변경해주면서 화면에 출력해보는 기능 추가 */
...
class StateAndEvent extends Component{
    ...
    StateChange=()=>{
        ...
        /* 
            ...
            setState 사용시 컴포넌트의 State값이 변경됐다고 간주하여서
            render 매서드를 자동으로 호출함        
        */
