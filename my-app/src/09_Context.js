/*
    ...
    단, 해당 데이터를 자식 컴포넌트가 아니라 후손 컴포넌트에서만 필요로 하는 데이터라면 자식컴포넌트에서는
    불필요하게 코드를 작성해줘야 하는데, 이때 Context를 사용하면 데이터 공급자와, 소비자를 정의하여
    데이터가 필요한 컴포넌트만 사용할 수 있게 구현할 수 있다
...
function ContextApi(){
    ...
    /*
        자식에게 넘겨줄 데이터에 상태값(name)과 상태값을 변경해줄 수 있는 함수를 넘겨줌
    */
    return (
